...
function cakes(recipe, available) {
  for (var ingredient in recipe) {
    if (!available[ingredient])
      return 0;
    var currentCount = Math.floor(available[ingredient] / recipe[ingredient]);
    if (!lowestPossible)
      var lowestPossible = currentCount;
    else if (currentCount < lowestPossible)
      lowestPossible = currentCount;
  }
